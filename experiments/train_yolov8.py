...
"""
Train YOLOv8 for Retail Item Detection with MLflow + DVC tracking.
...
def log_dvc_dataset_info(data_yaml: str):
    ...
    try:
        ...
        # Attempt to log referenced dataset paths in YAML
...
def parse_args():
    ...
    """
    p = argparse.ArgumentParser(description="Train YOLOv8 with MLflow+DVC")
    p.add_argument('--data', type=str, required=True, help='Dataset YAML file path')
    p.add_argument('--model', type=str, default='yolov8m.pt', help='Model weights or name')
    p.add_argument('--imgsz', type=int, default=640)
    p.add_argument('--epochs', type=int, default=100)
    p.add_argument('--batch', type=int, default=16)
    p.add_argument('--project', type=str, default='runs', help='Ultralytics project dir')
    p.add_argument('--name', type=str, default='yolov8_retail', help='Run name')
...
def main():
    ...
    with mlflow.start_run(run_name=f"train_yolov8_{Path(args.model).stem}"):
        # Log params
        mlflow.log_param("framework", "ultralytics_yolov8")
        ...
        if results_csv.exists():
            ...
            try:
                ...
                last = df.iloc[-1]
                # Common Ultralytics metrics columns
                for col in [
